"""
hetero_gat_visual_train.py

Versão experimental e ISOLADA do HeteroGAT, onde as features do nó Image
deixam de ser a média das emoções (10 dims) e passam a ser o embedding
visual reduzido por PCA (64 dims), extraído da ResNet50.

Tudo o resto mantém-se EXATAMENTE igual ao HeteroGAT original:
- Features do nó User (demográficas, one-hot)
- Emoções por interação como edge_attr nas arestas LIKED/DISLIKED
- Arquitetura do modelo (reutiliza a classe HeteroGAT original, sem alterações)
- Protocolo de avaliação (kfold5, early stopping, mesmas métricas)

Este script não modifica nem lê nada da pasta main\\GAT (só importa a classe
do modelo, que fica intacta) nem escreve em main\\results (os resultados
ficam isolados em resnet\\output).

Corre a partir de main\\resnet:
    python hetero_gat_visual_train.py
"""
...
PASTA_GAT    = os.path.join(PASTA_MAIN, "GAT")              # main\GAT (só para importar o modelo)
PASTA_OUTPUT = os.path.join(PASTA_SCRIPT, "output")
os.makedirs(PASTA_OUTPUT, exist_ok=True)

# main -> para importar split.py e evaluation.py (iguais ao original)
sys.path.append(PASTA_MAIN)
# GAT -> para importar a classe HeteroGAT original, sem a modificar nem duplicar
sys.path.append(PASTA_GAT)
...
import split
import evaluation
from hetero_gat_model import HeteroGAT  # classe original, reutilizada tal e qual

# ──────────────────────────────────────────────
# Configuração
# ──────────────────────────────────────────────

# --- INPUTS partilhados, lidos de main\data, NUNCA modificados ---
PATH_RATINGS = os.path.join(PASTA_MAIN, "data", "ratings_full.csv")
PATH_USERS   = os.path.join(PASTA_MAIN, "data", "users.csv")

# --- INPUT próprio (gerado pelo reduce_embeddings_pca.py, dentro de resnet\output) ---
PATH_VISUAL_PCA = os.path.join(PASTA_OUTPUT, "image_embeddings_pca64.csv")
...
# As emoções continuam a ser usadas nas ARESTAS (interação específica) — só
# deixam de ser usadas como feature do nó Image.
...
# ── Reaproveitar os mesmos hiperparâmetros já afinados para o HeteroGAT original
# (não estamos a re-fazer tuning para esta experiência isolada; o objetivo é
# isolar o efeito da troca de feature do nó Image, mantendo tudo o resto igual)
PARAMS_PATH = os.path.join(PASTA_MAIN, "results", "hetero_gat_best_params.json")
...
# ── ÚNICA MUDANÇA REAL: features do nó Image passam a ser a concatenação do
# embedding visual (PCA, 64 dims, estático) com a emoção média por imagem
# (10 dims). A parte de emoção é recalculada DENTRO do loop de fold, usando
# só train_df, para evitar reintroduzir a fuga de dados já corrigida no
# HeteroGAT original (ver secção "Data Leakage Correction").
visual_pca = pd.read_csv(PATH_VISUAL_PCA)
visual_pca = visual_pca.set_index('image_id')
visual_cols = [c for c in visual_pca.columns if c.startswith('visual_pca_')]
visual_features = visual_pca[visual_cols]   # estático, sem risco de fuga (não depende de ratings)

image_feat_dim = len(visual_cols) + len(EMO_COLS)   # 64 + 10 = 74

print(f"  Users: {ratings['user'].nunique()} | Items: {ratings['item'].nunique()}")
print(f"  Features demográficas (one-hot): {user_feat_dim}")
print(f"  Features do nó Image: {len(visual_cols)} visuais + {len(EMO_COLS)} emocionais = {image_feat_dim}")

# Aviso se alguma imagem do dataset de ratings não tiver embedding visual
items_sem_visual = set(ratings['item'].unique()) - set(visual_features.index)
if items_sem_visual:
    print(f"  AVISO: {len(items_sem_visual)} imagens sem embedding visual - vão usar vetor de zeros.")

# ──────────────────────────────────────────────
# 2. Construir HeteroData (igual ao original, exceto a fonte de image_features)
# ──────────────────────────────────────────────
...
# ──────────────────────────────────────────────
# 3. Loop kfold (igual ao original)
# ──────────────────────────────────────────────
...
for k, train_df, test_df in split.kfold_split(ratings, K_FOLDS, TOTAL_ITEMS, MIN_TEST, BASE_SEED):
    ...
    # ── Fusão por fold: emoção média calculada só a partir do train_df deste
    # fold (evita fuga de dados), depois concatenada com o embedding visual
    # estático (que não sofre de fuga, pois não depende dos ratings).
    emo_avg = train_df.groupby('item')[EMO_COLS].mean()
    all_items_this_fold = sorted(ratings['item'].unique())
    emo_avg = emo_avg.reindex(all_items_this_fold, fill_value=0.0)
    visual_aligned = visual_features.reindex(all_items_this_fold, fill_value=0.0)
    image_features = pd.concat([visual_aligned, emo_avg], axis=1)
    ...
    model = HeteroGAT(
        ...
        image_feat_dim=image_feat_dim,   # 64, em vez de 10
...
# ──────────────────────────────────────────────
# 4. Resultados finais — guardados isoladamente em resnet\output
# ──────────────────────────────────────────────
...
print("\n" + "="*50)
print("HeteroGAT-Visual (features do nó Image = PCA visual) — Resultados Finais (kfold5)")
print("="*50)
...
MODEL_NAME = "HeteroGAT_Visual_Fusion"

mean_prec_s = mean_prec.copy(); mean_prec_s.insert(0, 'model', MODEL_NAME)
mean_rec_s  = mean_rec.copy();  mean_rec_s.insert(0,  'model', MODEL_NAME)
mean_f1_s   = mean_f1.copy();   mean_f1_s.insert(0,   'model', MODEL_NAME)
mean_mrr_s  = mean_mrr.copy();  mean_mrr_s.insert(0,  'model', MODEL_NAME)

mean_prec_s.to_csv(os.path.join(PASTA_OUTPUT, "precision_heterogat_visual_fusion.csv"), index=False)
mean_rec_s.to_csv(os.path.join(PASTA_OUTPUT,  "recall_heterogat_visual_fusion.csv"),    index=False)
mean_f1_s.to_csv(os.path.join(PASTA_OUTPUT,   "f1_heterogat_visual_fusion.csv"),       index=False)
mean_mrr_s.to_csv(os.path.join(PASTA_OUTPUT,  "mrr_heterogat_visual_fusion.csv"),      index=False)
...
fixed_metrics.to_csv(os.path.join(PASTA_OUTPUT, "fixed_metrics_heterogat_visual_fusion.csv"), index=False)

print(f"\nResultados guardados isoladamente em: {PASTA_OUTPUT}")
print("Done!")