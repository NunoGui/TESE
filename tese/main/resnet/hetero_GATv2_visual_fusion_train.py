"""
hetero_gatv2_visual_train.py

Versão experimental e ISOLADA do HeteroGATv2, onde as features do nó Image
deixam de ser a média das emoções (10 dims) e passam a ser o embedding
visual reduzido por PCA (64 dims), extraído da ResNet50.

Tudo o resto mantém-se EXATAMENTE igual ao HeteroGATv2 original:
- Features do nó User (demográficas, one-hot)
- Emoções por interação como edge_attr nas arestas LIKED/DISLIKED
  (aqui entram nativamente na atenção via GATv2Conv, edge_dim=10 - isto NÃO muda)
- Arquitetura do modelo (reutiliza a classe HeteroGATv2 original, sem alterações)
- Protocolo de avaliação (kfold5, early stopping por NDCG@10, mesmas métricas)
...
Corre a partir de main\\resnet:
    python hetero_gatv2_visual_train.py
"""
...
# GAT -> para importar a classe HeteroGATv2 original, sem a modificar nem duplicar
...
import split
import evaluation
from hetero_gatV2_model import HeteroGATv2  # classe original, reutilizada tal e qual
...
PARAMS_PATH = os.path.join(PASTA_MAIN, "results", "hetero_gatv2_best_params.json")
...
# só train_df, para evitar reintroduzir a fuga de dados já corrigida.
...
visual_features = visual_pca[visual_cols]   # estático, sem risco de fuga

image_feat_dim = len(visual_cols) + len(EMO_COLS)   # 64 + 10 = 74

print(f"  Users: {ratings['user'].nunique()} | Items: {ratings['item'].nunique()}")
print(f"  Features user: {user_feat_dim} | Features image: {len(visual_cols)} visuais + {len(EMO_COLS)} emocionais = {image_feat_dim}")
...
def evaluate_model(model, test_df, user2idx, item2idx, x_dict, edge_index_dict, edge_attr_dict):
    """Avalia o modelo e retorna NDCG@10 médio para early stopping (igual ao original)."""
    model.eval()
    with torch.no_grad():
...
for k, train_df, test_df in split.kfold_split(ratings, K_FOLDS, TOTAL_ITEMS, MIN_TEST, BASE_SEED):
    ...
    # fold (evita fuga de dados), concatenada com o embedding visual estático.
...
print("\n" + "="*50)
print("HeteroGATv2-Visual (features do nó Image = PCA visual) — Resultados Finais (kfold5)")
print("="*50)
...
MODEL_NAME = "HeteroGATv2_Visual_Fusion"
...
mean_prec_s.to_csv(os.path.join(PASTA_OUTPUT, "precision_heterogatv2_visual_fusion.csv"), index=False)
mean_rec_s.to_csv(os.path.join(PASTA_OUTPUT,  "recall_heterogatv2_visual_fusion.csv"),    index=False)
mean_f1_s.to_csv(os.path.join(PASTA_OUTPUT,   "f1_heterogatv2_visual_fusion.csv"),       index=False)
mean_mrr_s.to_csv(os.path.join(PASTA_OUTPUT,  "mrr_heterogatv2_visual_fusion.csv"),      index=False)
...
fixed_metrics.to_csv(os.path.join(PASTA_OUTPUT, "fixed_metrics_heterogatv2_visual_fusion.csv"), index=False)
