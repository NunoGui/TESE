...
# ── ÚNICA MUDANÇA REAL: features do nó Image vêm do PCA visual, não das emoções médias
...
image_features = visual_pca[visual_cols]   # index = image_id, mesmas colunas para todas as imagens
image_feat_dim = len(visual_cols)          # 64, em vez de 10

print(f"  Users: {ratings['user'].nunique()} | Items: {ratings['item'].nunique()}")
print(f"  Features demográficas (one-hot): {user_feat_dim}")
print(f"  Features visuais (PCA) do nó Image: {image_feat_dim}")

# Aviso se alguma imagem do dataset de ratings não tiver embedding visual
items_sem_visual = set(ratings['item'].unique()) - set(image_features.index)
if items_sem_visual:
...
mean_prec_s.to_csv(os.path.join(PASTA_OUTPUT, "precision_heterogat_visual.csv"), index=False)
mean_rec_s.to_csv(os.path.join(PASTA_OUTPUT,  "recall_heterogat_visual.csv"),    index=False)
mean_f1_s.to_csv(os.path.join(PASTA_OUTPUT,   "f1_heterogat_visual.csv"),       index=False)
mean_mrr_s.to_csv(os.path.join(PASTA_OUTPUT,  "mrr_heterogat_visual.csv"),      index=False)
...
fixed_metrics.to_csv(os.path.join(PASTA_OUTPUT, "fixed_metrics_heterogat_visual.csv"), index=False)
