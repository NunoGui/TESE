...
# ──────────────────────────────────────────────
# 3. Treino e avaliação com kfold5 — otimiza MRR@10
# ──────────────────────────────────────────────
def train_and_evaluate_kfold(lr, weight_decay, n_layers):
    ...
    for k, train_df, test_df in split.kfold_split(ratings, K_FOLDS, TOTAL_ITEMS, MIN_TEST, BASE_SEED):
        ...
        mrr_folds.append(np.mean(mrr_list))

    return round(np.mean(mrr_folds), 4)
...
def objective(trial):
    ...
    mrr = train_and_evaluate_kfold(lr, weight_decay, n_layers)
    ...
    print(f"Trial {trial.number+1:2d}/{N_TRIALS} | lr={round(lr,5)} | wd={round(weight_decay,5)} | layers={n_layers} | MRR@10={mrr}")
    return mrr

print(f"\nA iniciar Optuna TPE Search — MRR@10 ({N_TRIALS} trials x {K_FOLDS} folds)...")
print("="*60)
...
print("\n" + "="*60)
print("RESULTADOS DO TUNING MRR@10 (kfold5)")
print("="*60)

results_df = pd.DataFrame(trial_results).sort_values("mrr@10", ascending=False)
...
print(f"  MRR@10:       {best_mrr}")

os.makedirs("results", exist_ok=True)
results_df.to_csv("results/lightgcn_tuning_mrr.csv", index=False)
with open("results/lightgcn_best_params_mrr.json", "w") as f:
