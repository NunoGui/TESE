"""
ABLATION STUDY: HeteroGATv2 sem features demográficas.

Este script é uma cópia isolada do hetero_gatV2_train.py já validado
(com a correção da fuga de dados e o patch de determinismo), com uma
única alteração: o vetor de features demográficas do utilizador (47
dimensões, one-hot) é substituído por um vetor de zeros.

Objetivo: isolar quanto (se algo) a informação demográfica contribui
para os resultados, mantendo arquitetura, hiperparâmetros e dados
exatamente iguais ao modelo original. A única variável que muda é
a presença ou ausência do sinal demográfico.

Os resultados são guardados com sufixo "_nodemo" para nunca
sobrescrever os resultados já validados do HeteroGATv2.
"""
...
# para que a única diferença entre este ablation e o modelo original seja a
# presença/ausência da demografia — não uma configuração diferente de tuning.
...
# ══════════════════════════════════════════════════════════
# ABLATION: zerar por completo o vetor demográfico.
# A dimensionalidade (user_feat_dim) mantém-se igual à do modelo
# original, para que a arquitetura (user_proj, hidden_dim, etc.)
# seja idêntica — só o CONTEÚDO da informação é removido.
# ══════════════════════════════════════════════════════════
demo_encoded.loc[:, :] = 0.0
print(f"  [ABLATION] Vetor demográfico ({user_feat_dim} dims) zerado para todos os utilizadores.")
...
print(f"\nA iniciar kfold ({K_FOLDS} folds)... [ABLATION: sem demografia]")
...
print("\n" + "="*50)
print("HeteroGATv2 [ABLATION: sem demografia] — Resultados Finais (kfold5)")
print("="*50)
...
# Guardar resultados (nomes com sufixo _nodemo para nunca sobrescrever os validados)
os.makedirs("../results", exist_ok=True)
mean_prec_s = mean_prec.copy(); mean_prec_s.insert(0, 'model', 'HeteroGATv2_NoDemo')
mean_rec_s  = mean_rec.copy();  mean_rec_s.insert(0,  'model', 'HeteroGATv2_NoDemo')
mean_f1_s   = mean_f1.copy();   mean_f1_s.insert(0,   'model', 'HeteroGATv2_NoDemo')
mean_mrr_s  = mean_mrr.copy();  mean_mrr_s.insert(0,  'model', 'HeteroGATv2_NoDemo')

mean_prec_s.to_csv("../results/precision_heterogatv2_nodemo.csv", index=False)
mean_rec_s.to_csv("../results/recall_heterogatv2_nodemo.csv",     index=False)
mean_f1_s.to_csv("../results/f1_heterogatv2_nodemo.csv",          index=False)
mean_mrr_s.to_csv("../results/mrr_heterogatv2_nodemo.csv",        index=False)

fixed_metrics = pd.DataFrame([{
    "model":        "HeteroGATv2_NoDemo",
...
fixed_metrics.to_csv("../results/fixed_metrics_heterogatv2_nodemo.csv", index=False)

print("\nResultados guardados em ../results/ (sufixo _nodemo)")
print("Done!")