...
# ── Carregar hiperparâmetros do tuning (se existirem)
...
print(f"  lr={LR} | weight_decay={WEIGHT_DECAY} | n_layers={N_LAYERS} | n_heads={N_HEADS}")
...
# ──────────────────────────────────────────────
# 2. Preparar features demográficas dos users (one-hot)
# ──────────────────────────────────────────────
...
# ──────────────────────────────────────────────
# 3. Preparar features emocionais das imagens (média)
# ──────────────────────────────────────────────
image_features = ratings.groupby('item')[EMO_COLS].mean()
image_feat_dim = len(EMO_COLS)
print(f"  Features emocionais das imagens: {image_feat_dim}")
 
# ──────────────────────────────────────────────
# 4. Função para construir HeteroData
# ──────────────────────────────────────────────
def build_hetero_graph(train_df, user2idx, item2idx, demo_encoded, image_features):
    data = HeteroData()
 
    # Nós User — features demográficas
    ...
    for u in user_ids_sorted:
        if u in demo_encoded.index:
            ...
            user_feat_matrix.append(np.zeros(demo_encoded.shape[1]))
    data['user'].x = torch.FloatTensor(np.array(user_feat_matrix))
 
    # Nós Image — features emocionais médias
...
for k, train_df, test_df in split.kfold_split(ratings, K_FOLDS, TOTAL_ITEMS, MIN_TEST, BASE_SEED):
    ...
    # Construir grafo
    graph = build_hetero_graph(train_df, user2idx, item2idx, demo_encoded, image_features)
    graph = graph.to(device)
 
    # Pares de treino para BPR
    train_pos     = train_df[train_df['rating'] == 1]
    train_pos_set = train_pos.groupby('user').apply(
        lambda x: set(item2idx[i] for i in x['item'] if i in item2idx)
    ).to_dict()
    train_pos_set = {user2idx[u]: s for u, s in train_pos_set.items() if u in user2idx}
    ...
    # Preparar edge_index_dict e edge_attr_dict
...
print("\n" + "="*50)
print("HeteroGAT — Resultados Finais (kfold5)")
print("="*50)
