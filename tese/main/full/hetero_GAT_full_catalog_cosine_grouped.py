...
fixed = pd.DataFrame([{
    "model":        "HeteroGAT_FullCatalog_Cosine_Grouped",
...
fixed.to_csv("../results/fixed_metrics_heterogat_full_cosine_grouped.csv", index=False)

diag_df = pd.DataFrame(all_diagnostics)
diag_df.to_csv("../results/diagnostic_heterogat_full_cosine_grouped.csv", index=False)

recs_df = pd.DataFrame(all_recommendations)
recs_df.to_csv("../results/recommendations_heterogat_full_cosine_grouped.csv", index=False)
